...
*/
int less(const Student *a, const Student *b) { return strcmp(a->id, b->id); }
 ...
 */
Elm *inserter(Elm *elm, Student *p) {
  if (elm == NULL) {
    Elm *temp = NULL;
  ...
  if (less(&elm->x, p) == 0) {
    ...
    elm->L = inserter(elm->L, p);
  } else {
    elm->R = inserter(elm->R, p);
  }
...
void rb_put(RBT *ptree, Student *px) {
  if (ptree->root == NULL) {
    ...
    ptree->root = inserter(ptree->root, px);
  }
}
...
/*
  ...
  Хэрэв мод тэнцвэрээ алдсан бол тохирох тэнцвэржүүлэх үйлдлүүдийг хийнэ.
 */
Elm *deletor(Elm *subtree, const char id[]) {
  if (subtree == NULL) {
    return NULL;
  }
  if (strcmp(subtree->x.id, id) == 0) {
    if (subtree->L == NULL && subtree->R == NULL) {
      ...
      subtree->x = temp->x;
      subtree->R = deletor(subtree->R, temp->x.id);
    }
  } else if (strcmp(subtree->x.id, id) < 0) {
    subtree->L = deletor(subtree->L, id);
  } else if (strcmp(subtree->x.id, id) > 0) {
    subtree->R = deletor(subtree->R, id);
  } else if (strcmp(subtree->x.id, id) < 0) {
    subtree->L = deletor(subtree->L, id);
  } else if (strcmp(subtree->x.id, id) > 0) {
    subtree->R = deletor(subtree->R, id);
...
void rb_del(RBT *ptree, const char id[]) {
  ptree->root = deletor(ptree->root, id);
}
